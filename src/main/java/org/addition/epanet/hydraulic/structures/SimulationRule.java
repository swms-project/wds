...
public class SimulationRule {
    ...
    public class Premise {
        ...
        public Premise(String[] Tok, Rulewords lOp, List<SimulationNode> nodes, List<SimulationLink> links) throws ENException {
            Objects loType;
            Varwords lVar;
            Object lObj;
            Operators lROp;
            ...
            loType = Objects.parse(Tok[1]);

            if (loType == Objects.r_SYSTEM) {
                lVar = Varwords.parse(Tok[2]);

                if (lVar != Varwords.r_DEMAND && lVar != Varwords.r_TIME && lVar != Varwords.r_CLOCKTIME)
                    throw new ENException(201);

                lObj = Objects.r_SYSTEM;
            } else {
                lVar = Varwords.parse(Tok[3]);
                if (lVar == null)
                    throw new ENException(201);
                ...
                switch (loType) {
                    ...
                    case r_RESERV:
                    case r_TANK:
                        loType = Objects.r_NODE;
                        break;
                    case r_LINK:
                    case r_PIPE:
                    case r_PUMP:
                    case r_VALVE:
                        loType = Objects.r_LINK;
                        break;
                    default:
                        throw new ENException(201);
                }

                if (loType == Objects.r_NODE) {
                    ...
                    switch (lVar) {
                        ...
                        case r_FILLTIME:
                        case r_DRAINTIME:
                            if (nodeRef instanceof SimulationTank) throw new ENException(201);
                            break;
            ...
            if (loType == Objects.r_SYSTEM)
                op = Operators.parse(Tok[3]);
            else
                op = Operators.parse(Tok[4]);
    ...
    public SimulationRule(Rule _rule, List<SimulationLink> links, List<SimulationNode> nodes) throws ENException {
        ...
        Rulewords ruleState = Rulewords.r_RULE;
        for (String _line : _rule.getCode().split("\n")) {
            String[] tok = _line.split("[ \t]+");
            Rulewords key = Rulewords.parse(tok[0]);

            if (key == null) throw new ENException(201);

            switch (key) {
                case r_IF:
                    if (!ruleState.equals(Rulewords.r_RULE))
                        throw new ENException(221);
                    ruleState = Rulewords.r_IF;
                    parsePremise(tok, Rulewords.r_AND, nodes, links);
                    break;

                case r_AND:
                    if (ruleState == Rulewords.r_IF)
                        parsePremise(tok, Rulewords.r_AND, nodes, links);
                ...
                case r_OR:
                    if (ruleState == Rulewords.r_IF)
                        parsePremise(tok, Rulewords.r_OR, nodes, links);
                    else
                        throw new ENException(221);
                    break;
                ...
                case r_PRIORITY: {
                    if (!ruleState.equals(Rulewords.r_THEN) && !ruleState.equals(Rulewords.r_ELSE))
                        throw new ENException(221);
    ...
    protected void parsePremise(String[] Tok, Rulewords logop, List<SimulationNode> nodes, List<SimulationLink> links) throws ENException {
